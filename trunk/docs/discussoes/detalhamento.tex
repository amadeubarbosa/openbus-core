...
as tarefas em três etapas (que não necessariamente serão implementadas
...
\subsection{Mecanismo de Leasing}

Um \textit{lease} é uma permissão de acesso a um recurso por um 
determinado período de tempo.
Um lease é negociado entre o usuário do serviço e seu provedor. Se o
lease não for renovado antes de expirar, o recurso pode ser liberado.
Isso pode ocorrer em caso de uma falha no cliente ou na rede, quando o
serviço não é mais necessário ou quando o lease não obteve
permissão para ser renovado.

No OpenBUS, o mecanismo de leasing é usado para controlar o acesso dos
componentes ao barramento. Através do serviço de acesso, os componentes
obtém uma credencial associada a um lease. Se esse lease expirar, a
credencial perde sua validade e o componente tem que obter uma nova
credencial. (Ver seção~\ref{acesso}.)
...
iniciou a aplicação. Servidores tipicamente se autenticam através de
um \textit{certificado}.
...
O serviço de acesso mantém uma tabela (Tabela~\ref{leasemap})
que mapeia o número de credenciais
ativas em validades mínimas e máximas a serem concedidas aos
componentes.

\begin{table}[htb]
\begin{center}
\begin{tabular}{c|c|c}
Número de Componentes	& Validade Mínima (ms)	& Validade Máxima (ms)\\
\hline \\
1 a 20 					& 300 					& 600 \\
20 a 100 				& 600 					& 1200 \\
$>$ 100 				& 1200 					& 1800 
\end{tabular}
\end{center}
\caption{Exemplo de um mapa de validade de leases.}
\label{leasemap}
\end{table}%

A cada vez que um componente renova sua credencial ele indica qual a
validade desejada e o serviço de acesso responde, com base em sua
tabela, a validade concedida.
Quando a validade de uma credencial expira, o componente perde o
acesso ao barramento. Qualquer componente pode pedir ao serviço de
acesso para ser notificado quando uma determinada credencial expirar.

Como é feita a ``limpeza'' desses observadores? Quando a credencial
deles expira eles são retirados da lista?!

Para evitar a perda de credenciais válidas, em caso de falha no serviço
de acesso, as credenciais são persistidas em um repositório de
credenciais.
Este repositório é atualizado pelo serviço de
acesso sempre que uma credencial é criada, removida ou quando sua 
validade é modificada.
Quando o serviço de acesso é iniciado, ele carrega todas
as credenciais que estão armazenadas no repositório.

Existe um módulo de persistência que é quem gerencia este repositório.
Ele mantém a validade de cada credencial em um arquivo, que tem como 
nome a própria credencial.
No futuro, este módulo pode ser trocado por um BD.

Um componente pode entregar sua credencial a outro
...
obteve a credencial do serviço de acesso, mas apenas um deles precisa 
renovar a validade da credencial.
...
Para que uma entidade seja autenticada através de
um certificado, o administrador do OpenBUS deve gerar um par de chaves
pública e privada. A chave privada deve ser entregue à entidade que deseja
acessar o barramento e a chave pública (certificado digital)
deve ser cadastrada no serviço de acesso.

\begin{itemize}

\item A entidade obtém o serviço de Acesso e executa o método
\textit{getChallenge}, passando o seu nome.

\item Através do nome da entidade, o serviço de Acesso obtém a chave
pública, que foi cadastrada anteriormente, e lhe envia um desafio.

\item A entidade recebe um desafio, que está criptografado com sua chave
pública.

\item A entidade deve abrí-lo com sua chave privada e
criptografá-lo com a chave pública do serviço de Acesso, gerando
uma resposta.

\item A entidade deve executar o método \textit{loginByCertificate} do serviço
de Acesso e enviar a reposta obtida no item anterior.

\end{itemize}

Observações:

Os serviços básicos são autenticados através de certificados e estes
certificados são bem conhecidos pelo serviço de Acesso. Em
princípio, serão cadastrados pelo administrador no momento da instalação
do barramento.

O serviço de Acesso deve possuir um certificado digital
...
e características/proprieda\-des de provedores desse serviço através
...
serviço associada(s) à credencial.
...
struct ServiceOffer {
    ...
    IMember member; /**< \brief O membro que está ofertando o serviço. */
};
...
interface IRegistryService {
     ...
     *
     * \param aCredential A credencial de quem está registrando a oferta.
     * \param aServiceOffer A oferta de serviço.
     *
     * \return Um identificador para o registro.
     */
    RegistryIdentifier register(in ACS::Credential aCredential,
      in ServiceOffer aServiceOffer);
     ...
     *
     * \param identifier O identificador do registro da oferta do serviço.
     * \param aserviceOffer A nova oferta de serviço.
     *
     * \return true, caso a oferta de serviço seja atualizada, ou false,
     * caso contrário.
     */
    boolean refresh(in RegistryIdentifier identifier,
      in ServiceOffer aServiceOffer);
 ...
 * - RegistryService
 */
interface IRegistryServiceComponent : IMember {
};
};
};
...
serviço de acesso. O componente ``gerador'' do grupo é responsável por
criar uma sessão. Ao receber uma credencial, ao invés de se autenticar
no serviço de acesso, o componente pede sua adição à sessão.
Componentes de um grupo, ao invés de se desconectarem do serviço de
acesso ({\em logout}) pedem a sua desconexão da sessão.

A renovação da credencial associada a uma sessão junto ao serviço de
acesso é feita pela sessão.
É a própria sessão quem implementa (para os membros do grupo) a
interface de renovação do leasing.
Apenas quando não há mais membros no grupo (dispostos a renovar a
credencial) ele deixa a credencial expirar. Neste caso, a sessão é
destruída.
O mecanismo de temporização do scheduler do OiL pode ser utilizado para
``ativar'' a renovação da credencial de cada sessão junto ao serviço de
acesso.  Isso partindo da premissa que esse mecanismo (implementado com
co-rotinas) não é custoso, e é escalável...
...
a credencial, ele elege outro componente para a tarefa.

A discutir: como proceder se a sessão (serviço de sessão) cair?

Um componente de uma sessão deve, obrigatoriamente, implementar
(faceta?) uma interface que permita responder à uma solicitação para
assumir a renovação da credencial/sessão e para receber mensagens
enviadas pela sessão.

Em princípio, estamos assumindo que as mensagens que trafegam em uma
sessão são ``broadcast''.
...
propriedades dos elementos (nós) que compõem essa hierarquia.
Uma propriedade de um elemento é um par nome X valor. Inicialmente,
...
interface DataService {
  Node getRootNode();
  Nodes getChildren(in NodeKey key);
};
};
};
\end{verbatim}
...
\item Nesse momento, o cliente 1 passa a manter sua credencial
      no serviço de sessão e é o serviço de sessão quem mantém esta
      credencial no serviço de acesso.
\item Cliente 1 entrega sua credencial para o cliente 2.
\item Cliente 2 obtém a referência para o serviço de registro através
      do serviço de acesso
\item Cliente 2 obtém a referência para o serviço de sessão através do
      serviços de registro.
\item Cliente 2 solicita ao serviço de sessão sua inclusão na sessão da
      credencial fornecida pelo cliente 1
\item Cliente 2 obtém, através do serviço de registro, um serviço de
      projetos, especificando seu tipo, e a presença de facetas de dados
      (navegação) e arquivo (acesso a conteúdo)
\item Cliente 2, utilizando a faceta de dados/navegação obtém a chave de
      um determinado dado de seu interesse.
\item Cliente 2 repassa, através de troca de mensagem, ao cliente 1, a
      chave do dado desejado.
\item Cliente 1 obtém via serviço de registro a referência para o
      serviço de projeto,
\item Cliente 1 solicita ao serviço de projeto a obtenção da referência
      para o dado a partir da chave fornecida pelo cliente 2.
\item Cliente 1 obtém o conteúdo do dado.
\item Cliente 1 finaliza a sessão
\item Serviço de sessão (sessão) faz o ``logout'' da credencial junto ao
      serviço de acesso
\end{itemize}
...
Cenário: Aplicação v3O2 utiliza serviço(s) de ``dados de poço'' para
obter (num primeiro momento) trajetórias de poços.

\begin{enumerate}

\item Usuário executa aplicação v3o2. Através de um menu, inicia
     procedimento para obter a trajetória de um poço.
  \begin{enumerate}
  \item Nesse momento a aplicação v3o2 se conecta ao barramento OpenBUS.
     É necessário autenticá-la. Para isso, serão pedidas identificação
     e senha do usuário.

     Uma vez conectada ao barramento, a aplicação v3o2 recebe uma
     credencial relativa a sua autenticação e pode obter a referência
     para o serviço de registro. Através do serviço de registro, ela tem
     acesso aos serviços básicos do barramento.

     A aplicação deverá validar periodicamente a sua credencial,
     através do mecanismo de leasing.
  ...
  \item A uma sessão será associado um canal (simples) de eventos. Os
     membros de uma sessão são, potencialmente, produtores e
     consumidores de eventos encaminhados por esse canal.

     -- MODELAGEM DOS EVENTOS QUE TRAFEGAM NO CANAL ASSOCIADO A UMA
     SESSÂO !!

  \item A conexão de um membro ao barramento não é suficiente para
     permitir sua ``localização'' (por exemplo, para encontrar membros
     de um determinado ``tipo'', convidá-los para uma colaboração...) A
     conexão permite apenas que se estabeleça uma sessão e que esse
     membro seja capaz de utilizar os serviços básicos do barramento, e
     também serviços ``exportados'' por outros membros (localizáveis
     através do serviço de registro)

     Um membro se tornará ``identificável/localizável'' se ``exportar''
     uma interface (faceta?) específica para isso.  Procedimento
     semelhante ao de uma oferta de serviço...
  \end{enumerate}
  
\item Assumindo que o identificador do poço não é conhecido, mas sim sua
     localização (geográfica?), o usuário pode lançar mão das
     facilidades de navegação do VGE para identificar o poço desejado.

  \begin{enumerate}
    \item Nesse momento, a aplicação v3o2 comanda a execução do VGE
     (via webstart).  O VGE receberá a credencial para permitir sua
     conexão (uso do barramento)

     Se o cliente VGE é assinado (provavelmente será), pode conectar-se
     diretamente ao barramento.

    \item O VGE deve disponibilizar (ao localizar um poço) uma opção de
     ``enviar evento'' através de sua sessão. O evento é o identificador
     do poço.  A partir de sua credencial, pode obter a referência para
     a sessão (canal).  Se a aplicação VGE termina, pode se desconectar
     do barramento.

     A aplicação v3o2 estava aguardando o evento com o identificador do
     poço.

     -- o usuário pode ``cancelar'' essa espera e finalizar todo o
     processo
  \end{enumerate}
  
\item v3o2 de posse do identificador do poço (obtido via VGE ou do
     próprio usuário, pulando etapa anterior) pode iniciar o
     procedimento de obtenção do dado propriamente dito (trajetória)

     Membro v3o2 pesquisa no serviço de registro ofertas de serviço de
     dados de poço com trajetória (trajetória é uma faceta? ou o serviço
     de poço é uma faceta, e ter trajetória é uma característica ?)

     Pode haver ``tipos'' diferentes para essa oferta: poço em
     perfuração (acesso a arquivos EXATA), poço ... (trajetória está na
     base)

     -- Como identificar esses tipos na publicação da oferta de serviço?

     Escolhida uma implementação (oferta) do serviço, v3o2 obtém os
     dados da trajetória do poço, fornecendo identificador do poço.

     -- Detalhar variante para identificação e acesso aos dados do poço
     via OpenSpirit.
\end{enumerate}
