...
--por certificado).
---
module "openbus.common.ServerConnectionManager"
...
function connect(self, name, leaseExpiredCallback)
  ...
  if not challenge then
    log:error("ServerConnectionManager: o desafio para "..name..
  ...
  if not privateKey then
    log:error("ServerConnectionManager: erro obtendo chave privada para "..
...
function reconnect(self)
  log:conn("ServerConnectionManager: reconectando "..self.name)
  if self:connect(self.name, self.leaseExpiredCallback) then
    log:conn("ServerConnectionManager: "..self.name.." reconectado")
    if self.leaseExpiredCallback then
      self.leaseExpiredCallback()
    end
  else
    log:error("ServerConnectionManager: insucesso na reconexão de "..self.name)
  end
end
