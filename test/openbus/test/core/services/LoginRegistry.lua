...
function LoginsFixture:subscribeLoginObs()
  local context = self.openbus.context
  -- create new observer
  local observer = newObserver({ entityLogout = true }, context)
  -- subscribe a new observer and validate some of its operations
  local subscription = self.logins:subscribeObserver(observer)
  local login = context:getCurrentConnection().login
  checks.assert(subscription, isLoginSubscription(observer, login))
