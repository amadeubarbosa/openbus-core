...
Para concluir o processo de login por certificado é necessário chamar a operação \code{LoginByRegistry::login} do objeto devolvido na etapa anterior, fornecendo uma chave pública de acesso (\code{pubkey}) e um bloco de dados encriptados (\code{encrypted}) com a chave pública do barramento, que é obtida através do atributo \code{buskey} desta mesma interface.
...
Em seguida desses dois bytes deve vir uma codificação CDR (encapsulado) da estrutura abaixo:
...
Em seguida, a seguinte estrutura deve ser codificada em CDR (encapsulado):
