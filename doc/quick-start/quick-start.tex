...
Caso tenha interesse de entender melhor o que é um barramento \openbus{} e os detalhes de instalação, consulte o seu manual de referência~\cite{ob2.0core} e o manual de instalação~\cite{ob2.0install}. Os procedimentos a seguir descrevem a instalação em plataforma Linux (Ubuntu 12.04) utilizando comandos do Bash shell (interpretador de linguagem de comandos)~\cite{web:bash}.
...
\end{verbatim}
  ...
  \item Inclua a seguinte variável de ambiente no seu arquivo de inicialização. Não esqueça de sair e entrar novamente no shell para que a variável de ambiente seja definida para a sessão corrente.
\begin{verbatim}  
  $ cd ~
  $ vi .bashrc 
...
\end{verbatim}
  ...
  \item Gere um arquibo de chave privada (arquivo com terminação .key) e um certificado (arquivo com terminação .crt) utilizando o comando openssl que acompanha o pacote:
\begin{verbatim}  
  $ cd $OPENBUS_HOME
...
\end{verbatim}
  ...
  \item Criei um script validador em LUA para verificar se o nome do usuário e senha utilizados para acessar o barramento são válidos. No nosso caso, vamos utilizar um script validador que verifica apenas se o nome do usuário é igual à senha:
\begin{verbatim}  
  $ cd $OPENBUS_HOME
  $ vi validador.lua
