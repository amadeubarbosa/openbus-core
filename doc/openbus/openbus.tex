...
Na seção~\ref{sec:architecture} apresentamos a arquitetura do sistema, e na seção~\ref{sec:perspectives} falamos um pouco mais sobre o projeto de acordo com a visão de cada tipo de usuário.
Por fim, na seção~\ref{sec:glossary} apresentamos um glossário com os conceitos principais do projeto.
...
O formato do script \lua{} que pode ser passado para os comandos``script'' e ``undo-script'' deve seguir o formato especificado no código~\ref{lst:openbus.adm}.
