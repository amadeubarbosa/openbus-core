...
\begin{description}
   \item[businit] script de inicialização para iniciar e parar o barramento, semelhante aos scripts do /etc/init.d em servidores Unix.
   \item[buscheck] script que verifica se o barramento está executando e, caso não esteja, inicia o barramento utilizando o script \emph{businit}.
\end{description}

\subsection{Inicialização}

Recomendamos que o administrador coloque o script \emph{businit} em /etc/init.d/openbus (ou outro lugar semelhante em seu sistema operacional) e faça o link simbólico apropriado para cada \textit{runlevel}. É possível editar o script \emph{businit} para redefinir o local da instalação do barramento (variável \code{INSTALL_DIR}), de modo que o script possa encontrar o executável do \emph{busservices}.
Alternativamente, é possível também definir a variável de ambiente \code{OPENBUS_HOME} para indicar o local de instalação do barramento para os scripts \code{businit} e \code{buscheck} (veja a seção seguinte).
...
Assim como o script \code{businit}, o script \code{buscheck} também precisa ser alterado para indicar o local de instalação do barramento(variável \code{INSTALL_DIR}).
Também como o script \code{businit}, o script \code{buscheck} também carrega configurações do arquivo \code{/etc/default/openbus}, ou do arquivo \code{config/openbus.rc} do diretório de instalação do barramento.

Como o script \textbf{depende} do script \emph{businit}, caso o administrador decida colocar o \emph{businit} em um local particular, \textbf{é importante editar o script \emph{buscheck} de acordo} (variável \code{OPENBUS\_INIT}).

Um exemplo de agendamento no \emph{cron} para utilizar o \emph{buscheck} é dado a seguir, neste exemplo consideramos que o barramento está instalado em /opt/openbus-2.0:
\begin{verbatim}
...
O comando \textbf{logrotate} é um utilitário para simplificar a administração de arquivos de logs bem comum na maioria das instalações Linux e Solaris.
O logrotate permite a rotação automática, a compressão dos logs antigos, a remoção de logs muito velhos e mesmo o envio de emails contendo os arquivos de logs.
...
Um exemplo simples de configuração de um validador personalizado, seria criar um código como o apresentado em \ref{subsec:valpersonalizados}, e salvar o código em um arquivo com a extensão \code{.lua}. Depois, adicione o caminho onde foi salvo o arquivo à variável de ambiente LUA\_PATH, da seguinte forma: 
