...
\begin{itemize}
  ...
  \item Executar o processo servidor da classe \$OPENBUS\_HOME/demo/hello/java/src/tecgraf/openbus/demo/hello/HelloServer.java.
  \item Executar o processo cliente da classe \$OPENBUS\_HOME/demo/hello/java/src/tecgraf/openbus/demo/hello/HelloClient.java. Após a execução do cliente, no terminal do processo servidor a seguinte mensagem deve ser exibida: "Hello !!!"
\end{itemize}
...
// da Listagem 1. Ambos serão passados ao ORB em sua inicialização.
bus.resetAndInitialize(args, props, "localhost", 2089);
\end{labeledcode}

Um ORB e POA são criados implicitamente na chamada \emph{resetAndInitialize}.
...
\begin{itemize}
  ...
  \item \code{void finish(boolean force)}: Finaliza a execução do ORB.
  ...
  \item \code{void setInterceptedCredentialSlot(int)}: Define o slot da credencial da requisição atual.
  ...
  \item \code{boolean addLeaseExpiredCallback(LeaseExpiredCallback lec)}: Registra uma callback para a notificação de que o lease da credencial	de identificação do usuário expirou.
...
Um exemplo do código necessário para a conexão com o barramento pode ser visto na Listagem 3.
...
Como o servidor, o cliente necessita de se autenticar ao barramento. Portanto, os mesmos passos aplicados ao servidor para a autenticação devem ser repetidos, com exceção de que o cliente geralmente se autentica através do mecanismo de usuário e senha, como na Listagem 9.
