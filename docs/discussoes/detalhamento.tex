...
Ele é responsável por válidar os componentes
que querem fazer parte do barramento, seja como clientes ou como
servidores. Essa válidação pode ser feita através de um par
\textit{usuário,senha} ou através de um \textit{certificado}.
Esse serviço é implementado em Lua e usa as bibliotecas OiL,
openSSL e luaLDAP.

Após fazer uma validação, o serviço de acesso emite uma
credencial que deverá ser usada pelo componente para
...
a referência para os demais serviços básicos. Já os serviços
não-básicos, devem ser obtidos através do serviço de registro.
...
\footnotesize
\begin{verbatim}
  CONEXÂO DE SERVIDORES
  ========================================================

  1) Não há distinção entre clientes e servidores (provedores de serviço).
     Todos são membros do barramento, autenticados na conexão, e estão
     no registro.  (diretório).
...
pode retirar o componente se seu repositório sem que este 
...
serviço de acesso. Um dos componentes é ``eleito'' pelo serviço
para manter a credencial ativa. Se o serviço de sessão nota que
a credencial expirou, ele eleje outro componente para a tarefa, mas
enquanto isso, renova a credencial no serviço de acesso.
Apenas quando não há mais membros no grupo ele deixa a credencial
expirar.
...
\item cliente 1 obtém, através do serviço de acesso, a referência
      para o serviço de sessão e cria uma sessão para sua credencial.
