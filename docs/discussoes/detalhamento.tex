...
Explicar hierarquia.
Presta serviço através de facetas.
...
\item A entidade obtém o Serviço de Controle de Acesso e executa o método \textit{getChallenge}, passando o seu nome.

\item O desafio recebido está criptografado com a chave pública da entidade. O Serviço de Controle de Acesso obtém essa chave a partir do certificado digital da entidade que foi cadastrado anteriormente. O cadastro de certificados digitais deve ser realizado externamente, pelo administrador.

\item Ao receber o desafio, a entidade deve abrí-lo com sua chave privada e criptografá-lo com a chave pública do Serviço de Controle de Acesso, gerando uma resposta.

\item A entidade deve executar o método \textit{loginByCertificate} do Serviço de Controle de Acesso e passa a reposta obtida no item anterior.
...
\item Os serviços básicos são autenticados através de certificados e estes certificados são bem conhecidos pelo Serviço de Controle de Acesso. Em princípio, serão cadastrados pelo administrador no momento da instalação do barramento.

\item O Serviço de Controle de Acesso deve possuir um certificado digital acessível por todos os membros.
...
Este serviço também é implementado em Lua

\subsubsection{Ofertas de Serviço}
Uma oferta de serviço tem um "tipo", uma descrição, um conjunto (opcional) de
propriedades e a referência para o componente (membro) que o implementa.

"Tipo" define qual o serviço oferecido (por exemplo, serviço de projetos,
serviço de dados de poço, etc...)

Dúvida: O tipo é só um nome arbitrário, ou deveria, de alguma forma, indicar 
a interface implementada? Por exemplo, no caso do serviço de sessão, discutiu-se
que sua funcionalidade
seria implementada por uma interface "não faceta"?
Neste caso, por exemplo, só um nome arbitrário é uma "amarração" suficiente para 
a interface do serviço oferecido?

No caso de um serviço de poço, a funcionalidade estaria implementada via suas
facetas. Neste caso, uma faceta é amarrada com uma interface...

E um serviço de projetos? Oferece funcionalidade apenas via facetas?

"Descrição" é um texto que pode ser exibido em uma interface para que um
usuário possa, por exemplo, decidir entre ofertas de serviço que tenham o 
mesmo tipo e propriedades equivalentes.

Propriedades de uma oferta de serviço são pares nome X valor.
...
Uma outra propriedade possível é o tipo de "fonte" dos dados (Base integrada,
OpenSpirit). Essa propriedade permitiria uma seleção "programática" de uma
oferta de serviço de dados com base na fonte desses dados.
...
Outro critério (opcional) são valores das propriedades. Neste caso,
temos dois filtros para uma oferta: a existência da propriedade,
...
Se, e quando, precisarmos de tipos de valores diferentes, é necessário podermos
checar casamento do tipo fornecido pelo importador da oferta com o tipo
fornecido pelo exportador. Será necessário, então, prever um "cadastramento" de
tipos de serviço, com descrição de propriedades, como o trader faz?

O resultado de uma busca é o conjunto de ofertas que o atende (conforme cadastrado
pelos exportadores dessas ofertas)

Dúvida: Como será feita a busca pelas ofertas de serviço? Consultas a tabelas
Lua?

Um exportador de oferta de serviço pode alterar suas ofertas (alterar
propriedades, por exemplo?) e também removê-las integralmentes.
O serviço de registro associará um identificador para uma oferta;
...
E os observadores que ficaram no serviço de acesso?
