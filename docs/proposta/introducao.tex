...
desenvolvidas em parceria com universidades ou compradas de terceiros. Durante
a realização de seu fluxo de trabalho, o funcionário pode ter a necessidade de
utilizar mais de uma aplicação e, embora essas aplicações trabalhem bem
individualmente, qualquer troca de dados deve ser realizada manualmente pelos
funcionários (os usuários das aplicações).

Para diminuir a participação do usuário no papel de integrador, a unidade de
Exploração e Produção precisa integrar todas as aplicações utilizadas pelos
usuários nos seus fluxos de trabalho. Algumas aplicações já estão oferencendo
mecanismos para que outras aplicações acessem seus dados. Como exemplo temos o
WebSintesi e o Sigeo que permitem acesso às suas áreas de projeto. O
WebSintesi disponibiliza uma biblioteca para acesso à sua área de projetos
enquanto que o Sigeo possui um servidor que possibilita às aplicações gravarem
dados em sua área de projetos.

Algumas aplicações especiais, os chamados sistemas integradores, vêm sendo
utilizadas para facilitar a integração de aplicações. Esses sistemas provêem
...
integrados. O WebSintesi acessa dados disponibilizados pelo OpenSpirit.

Nesse contexto, podemos apontar algumas dificuldades:

\begin{itemize}
\item {\bf O usuário precisa assumir o papel de integrador enquanto realiza o seu fluxo de trabalho.}

O fluxo de trabalho de um funcionário torna-se mais complexo pois, além de ser
usuário das aplicações, este funcionário deverá atuar também como integrador
das aplicações. É provável que este funcionário precise conhecer detalhes das
aplicações que são transparentes para um usuário não-integrador.
...
\item {\bf Integrar aplicações é uma tarefa inerentemente complexa.}
...
são alguns dos aspectos que tornam a integração de aplicações uma tarefa
...
O integrador comercial em uso na unidade de Exploração e Produção
atualmente é o OpenSpirit. Como veremos mais adiante, o OpenSpirit possui
algumas limitações que impedem que as aplicações desenvolvidas pela Petrobras
sejam integradas através de sua infraestrutura.

Além disso, adotar soluções fechadas pode trazer alguns inconvenientes:
\begin{itemize}
...
A seção 2 apresenta um estudo sobre as ferramentas e tecnologias disponíveis para a implementação de uma solução. A seção 3 apresenta os requisitos que devem ser atendidos pela solução e a arquitetura proposta. Finalmente, a seção 4 apresenta as conclusões.
