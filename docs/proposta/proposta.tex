...
Conforme discutimos na seção anterior, duas ferramentas de mercado objetivam
...
Já no caso do PowerHub, há duas questões técnicas impeditivas: o fato desta
 ferramenta não prover suporte para a troca de alguns tipos de dados---que é o
 objetivo principal da integração desejada---e o fato de estar restrita à
 linguagem Java.
...
Motivos possíveis para uma eventual interrupção do uso são o aumento dos custos
 das licenças, a descontinuidade da ferramenta por parte do fabricante ou mesmo
...
Em função dessas questões, nós propomos a construção de uma nova arquitetura de
 integração de aplicações, baseada em um barramento de serviços, própria da
...
\begin{description}
\item [Arquitetura aberta e extensível a diferentes domínios.] \hfill O 
\hfill E\&P \hfill abrange \hfill várias \\
 áreas de conhecimento além de geologia e geofísica. Além disso,
...
\item [Suporte a mecanismos de autenticação e autorização.] \hfill Em 
\hfill praticamente \hfill qualquer \\
área da Petrobras, a segurança da informação é essencial. O
 \openbus\/ deve, então, dar suporte para que a autenticação dos usuários e a
 autorização de acesso a dados e serviços sejam feitos conforme necessário.
...
Como apresentamos na seção anterior, COR\-BA possui todos os requisitos para uma
 implementação de SOA.
Além disso, CORBA oferece suporte para descrição clara e sucinta das interfaces
 dos serviços, à construção de componentes em múltiplas linguagens, à
 mensageria, à implementação de mecanismos de autenticação e ao acesso, via
 referências, a objetos remotos que podem preservar estado.
