...
Já no caso do PowerHub, há duas questões técnicas impeditivas: o fato desta
 ferramenta não prover suporte para a troca de alguns tipos de dados---que é o objetivo principal da
...
Em função dessas questões, nós propomos a construção de uma nova arquitetura de
 integração de aplicações, baseada em um barramento de serviços, própria da Petrobras, que
 chamamos de \openbus.
...
\item [Suporte a mecanismos de autenticação e autorização.] Em praticamente
 qualquer área da Petrobras, a segurança da informação é essencial. O \openbus\/
...
Como apresentamos na Seção \ref{CORBA}, CORBA possui todos os requisitos para
 uma implementação de SOA.
Além disso, CORBA oferece suporte para descrição clara e sucinta das interfaces dos
