...
baseado em JMX ({\em Java Management Extensions}).
...
O foco na integração de aplicações e sistemas desenvolvidos em Java
e o favorecimento de ambientes de um alto nível de integração
--- são encontradas em algumas infraestruturas comerciais para
a construção de barramentos de serviços, como o {\em AquaLogic Service Bus} da BEA.
Assim como a arquitetura JBI, essa infraestrutura provê um mecanismo de
roteamento de mensagens que representam requisições de serviços descritos
em WSDL. Esses serviços são oferecidos por dois tipos de componentes:
\begin {itemize}
\item {\em Business Services} --- serviços corporativos e de acesso a bancos de
dados conectados diretamente ao barramento de serviços.
em um servidor WebLogic.
\item {\em Proxy Services} --- intermediários para provedores de serviços externos.
\end{itemize}
...
CORBA não exclui SOA nem mensageria,
mas agrega vantagens e facilidades de objetos distribuídos

Atende a diversos cenários de integração.
...
serviço de notificação suporta interações assíncronas e desacoplamento entre componentes (caso da colaboração)

É a infraestrutura de integração adotada pelo OpenSpirit,
que atende aos requisitos do nosso ambiente de integração
(a menos da possibilidade de integrar servidores próprios)
