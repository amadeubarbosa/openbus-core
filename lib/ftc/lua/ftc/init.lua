...
--[[VERBOSE]] LOG:open("socket criado...")
  local status, errmsg = self.channel:connect(self.host, self.port)
  if not status then
    return nil, errmsg
  end
--[[VERBOSE]] LOG:open(string.format("socket conectado em %s:%d", self.host, self.port))
  self.buffer = string.char(string.len(self.accessKey))
  self.buffer = self.buffer..self.accessKey
-- [[VERBOSE]] LOG:open("accessKey = ", self.accessKey," buffer = ", verbose.bytes(self.buffer))
...
-- [[VERBOSE]] LOG:open("identifier = ", self.identifier," buffer = ", verbose.bytes(self.buffer))
...
--[[VERBOSE]] LOG:open("OpenCode = ", code)
  if errorCode[code] then
--[[VERBOSE]] LOG:open("Error:", code)
    return nil, errorCode[code], code
  end
  self.opened = true
...
-- [[VERBOSE]] LOG:setPosition("position = ", position, " buffer = ", verbose.bytes(self.buffer))
...
-- [[VERBOSE]] LOG:read("nbytes = ", nbytes," position = ", position, " buffer = ", verbose.bytes(self.buffer))
...
function write(self, nbytes, position, data)
  ...
  local available = self.size - position
--print(nbytes, available)
  if ( nbytes > available ) then
    nbytes = available
  end
  self.buffer = self.buffer..LuaNumber2Long(position)..LuaNumber2Long(nbytes)
-- [[VERBOSE]] LOG:write("nbytes = ", nbytes," position = ", position, " buffer = ", verbose.bytes(self.buffer))
